(define-module (ui verify-unmapped)
  ...
  #:use-module (pcawg verify-unmapped)
  #:use-module (srfi srfi-1)

  #:export (do-verify-unmapped))
...
(define (donor->verify-unmapped-reads bucket storedir donor-id)
  ...
  (define (extract-unmapped suffix)
    (let* ((full-id     (string-append donor-id suffix))
      ...
      (if (not (file-exists? done-file))
          (log-debug "donor->verify-unmapped-reads"
                     "~s hasn't been post-processed yet." full-id)
          (receive (success? total-unmapped observed-unmapped)
              (verify-unmapped-reads input-file output-file)
            (if (not success?)
                (begin
                  (log-error "donor->verify-unmapped-reads"
                             "Error: ~s" total-unmapped)
                  #f)
                (begin
                  (log-debug "donor->unmapped-reads"
                             "~s has ~a reads without a coordinate, and ~a were found."
                             full-id total-unmapped observed-unmapped)
                  #t))))))
    ...
    (let ((donor (car donors)))
      (process-jobs (cdr donors) number-of-jobs report-bucket store-directory
                    (cons (call-with-new-thread
                           (lambda _
                             (donor->verify-unmapped-reads
                              report-bucket store-directory donor)))
                          threads))))))

(define (do-verify-unmapped options)
  (let ((config (getopt-long options program-options)))
    ...
    (log-debug "verify-unmapped" "Started.")
    (log-error "verify-unmapped" "Started.")
